...
# ---------------------------------------------------------------------------
# OntologyQuery — cross-ontology keyword search
# ---------------------------------------------------------------------------
...
def _make_ontology_query(store: OntologyStore) -> Executor:
    def run(params: dict, cwd: str) -> str:
        ...
        sections: list[str] = [f"# OntologyQuery: {query!r}"]
        labels = {
            "terms": "术语 (Terms)",
        ...
        if total == 0:
            return f"OntologyQuery: no matches for {query!r}."
        return "\n".join(sections)
    return run


# ---------------------------------------------------------------------------
# TermDisambiguate — resolve a single term to its canonical entry
# ---------------------------------------------------------------------------

TERM_DISAMBIGUATE_SCHEMA = {
    "name": "TermDisambiguate",
    "description": (
...
def _make_term_disambiguate(store: OntologyStore) -> Executor:
    def run(params: dict, cwd: str) -> str:
        ...
        if not hits:
            return f"TermDisambiguate: no term matches {term!r}."
        if len(hits) == 1:
            return hits[0].to_prompt()
        lines = [f"TermDisambiguate: {len(hits)} candidates for {term!r}:"]
        for t in hits:
...
# ---------------------------------------------------------------------------
# MetricLookup — full metric spec including SQL hints + dims
# ---------------------------------------------------------------------------
...
def _make_metric_lookup(store: OntologyStore) -> Executor:
    def run(params: dict, cwd: str) -> str:
        ...
        if not hits:
            return f"MetricLookup: no metric matches {metric!r}."
        if len(hits) == 1:
            return hits[0].to_prompt()
        lines = [f"MetricLookup: {len(hits)} candidates for {metric!r}:"]
        for m in hits:
...
# ---------------------------------------------------------------------------
# RelationLookup — relations touching a given LE (for join planning)
# ---------------------------------------------------------------------------
...
def _make_relation_lookup(store: OntologyStore) -> Executor:
    def run(params: dict, cwd: str) -> str:
        ...
        if not le:
            return f"RelationLookup: no logical entity matches {entity!r}."
        relations = store.relations_of(le.code)
        if not relations:
            return f"RelationLookup: {le.name} ({le.code}) has no recorded relations."
...
# ---------------------------------------------------------------------------
# EntityDescribe — full LE context: definition, attributes, relations
# ---------------------------------------------------------------------------
...
def _make_entity_describe(store: OntologyStore) -> Executor:
    def run(params: dict, cwd: str) -> str:
        ...
        le = store.get_logical_entity(entity)
        if not le:
            return f"EntityDescribe: no logical entity matches {entity!r}."
        lines = [le.to_prompt()]
